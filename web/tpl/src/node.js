...
var editor = new LiteGraph.Editor("main",{miniwindow:false,skip_livemode:true});
...
//test
//editor.graphcanvas.viewport = [200,200,400,400];x
...
elem.innerHTML = "";
elem.innerHTML += "常用文件 <select><option>请选择</option></select> <button class='btn' id='save'>Save</button><button class='btn' id='load'>Load</button>";
...
$.get('/api/ui/templates', r=>{
    r.data.forEach(e => {
        addDemo(e.name, '/api/ui/template/'+e.name);    
    });
})
